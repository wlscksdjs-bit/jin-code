...
export default function ImportTimeSheetsPage() {
  ...
  const downloadTemplate = async () => {
    const res = await fetch('/api/import/timesheets')
    const blob = await res.blob()
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'timesheets-template.xlsx'
    a.click()
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">근태 일괄 등록</h1>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Excel 파일로 근태 일괄 등록</CardTitle>
        </CardHeader>
