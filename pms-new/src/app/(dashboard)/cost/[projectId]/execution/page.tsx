...
export default function CostExecutionFormPage({ params }: { params: Promise<{ projectId: string; id?: string }> }) {
  const session = auth()
  if (!session) redirect('/signin')

  return <CostExecutionForm params={params} />
}

function CostExecutionForm({ params }: { params: Promise<{ projectId: string; id?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={async (formData) => {
        ...
        const { submitCostExecution } = await import('./actions')
        formData.set('fabricationVendors', JSON.stringify(fabVendors))
        formData.set('serviceVendors', JSON.stringify(svcVendors))
        await submitCostExecution(formData, (await params).projectId, (await params).id)
      }} className="space-y-6">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/cost"><Button variant="outline" type="button">취소</Button></Link>
        </div>
      </form>
    </div>
  )
